 ...
 * 			=> Mac, Windows , Linux , 안드로이드(모바일) => 달빅
 * 			=> .class => 모든 운영체제에서 사용이 가능
 * 		2) 객체지향 프로그램 (재사용 => 자바(유지보수) => 대규모 프로그램)
 * 			=> 캡슐화, 은닉화, 상속(is-a) , 포함(has-a) , 다형성 (오버로딩(new) , 오버라이딩(modify))
 * 			단위 클래스
 * 			=> 자바 구조
 * 			1.package (1번사용)
 ...
 * 				  => 메인보드 (CBD)
 * 		3) 비교적 배우기 쉽다
 * 			(C/C++ 에서 파생된 언어) => 가장 어려운 부분인 포인터(메모리 주소), 구조체 , 직접 제작 
 * 												============================== (X) 없앴다
 ...
 * 		4) 자동 메모리 관리 (GC) 
 * 		-----------------------------------------------------------------------------
 * 		5) 네트워크 / 분산 / 멀티 쓰레드 / 동적로딩
 * 		   웹서버 (tomcat)
 * 		   Web에서 사용 C/S (Client/Server)
 * 						  --------------
 * 						  Front   Back(O) : 장점)변경이 거의 없다.
 * 						 ----------------------------------- Full Stack
 * 						 ------- React (1주일 단위 버전이 변경)
 * 									=> 자주 사용하는 함수 (변경)
 * 		 질 보다는 양
 ...
 * 			3. 특수문자를 사용할 수 있다 (_ , $)
 * 			4. 키워드(예약어)는 사용할 수 없다 (자바에서 사용하는 단어)
 * 				=> 빨간색으로 만들어진것은 키워드
 * 			------------------------------ 길이는 제한이 없다 (3~7자 사이)
 * 			약속 (가독성)
 ...
 * 		자바에서 지원하는 기본형
 * 		1) 정수 저장 공간
 * 			= byte (1byte 저장 => -128~127) => 범위를 벗어나면 오버플로우 발생
 * 			  오버플로우 -5~5
 * 			= short (2byte => -32768~32767) => C언어 호환성
 * 			= int (4byte => 32bit) => -21억4천~21억4천
 ...
 * 				  10 => int
 * 				  10L => long
 * 				**default => int(커뮤터에서 인식하는 정수는 int)
 * 		2) 실수 저장 공간
 * 			= float(4byte) ==> 소수점 5자리
 ...
 * 		  byte b;
 * 		  int a;
 * 		  long a; //8byte 정수 공간을 만들어 달라, 이름 a
 * 		  double d; // 8byte 메모리 공간을 만든다, 공간의 이름을 d라고 한다
 * 		  char c; // 문자를 저장하는 공간을 만들어라, 찾을 경우 c 라는
 *   	======================================
 *   	  실제 값을 저장
 *   	----------리터럴
 *   	변수, 상수 , 리터럴
 ...
 *   		  a=10;
 *   		= 입력값을 받아서 저장
 *   		--------Scanner 를 이용하는 방식 (키보드에서 입력된 값을 메모리에 저장)
 *   				-----------------------------------------------
 *   		------- IO (BufferedReader , System.in) => 예외처리
 *   		= 난수 (랜덤) Math.random() , Random
 *   		----------------------------------
 *   		=파일을 읽어서 초기화
 *   		=***데이터베이스(오라클) 읽어서 초기화 (CRUD)
 *   			C (Create) : 추가 => 글쓰기
 *   			R (Read) : 검색 => 목록 / 상세
 *   			U (Update) : 수정 => 글수정
 *   			D (Delete) : 삭제 => 삭제
 ...
 *   	임시변수 temp
 *   
 *   	그냥 a=b , b=a 라고 하면 둘다 20으로 나옴. a 의 값을 temp 로 옮긴후 교환을 진행해야함
 *   
 *   	Web은 문자만 사용
 *   	char c1 = 'A'
 *   	
